...
| **产品页（单文件）** | `svgb_beautifier.html` | 7 425 行 / 1 578 081 B，由 `build.py` 内联全部依赖，双击即用 |
| 核心实现 | `src/01_util.js` … `11_realsvgs.js` | 11 个模块，共 387 201 B，见 §2 |
...
路径改写覆盖 86 个文件（`labs/svgb/` → 空、`"../../vendor/` → `"vendor/`、`labs/build.py` → `build.py`、
`report/assets/evidence_` → `assets/evidence_`，以及 `tests/*.cjs` 的用法注释）。终态残留检查：
`labs/svgb` 命中 **0**、`../../vendor` 命中 **0**。

### 20.2 构建脚本里修掉的一个真缺陷 + 一条非显然行为

`build.py` 早期版本把每段体积打成了 `len(js)` —— 那是**码点数**，不是字节数。
源码混有大量 CJK，两者差 1.2~1.4 倍（`src/09_editor.js`：**57 099 码点 / 71 541 B**），
标成 `B` 会让人误读体积。现在两个数都打，并新增 `--verify`。

同时实测确认了一条**非显然行为**：`read()` 走 Python 通用换行，所以 **CRLF 源内联进产物后变 LF**。
本目录里 `src/06_geometry.js` 是**唯一**全 CRLF 的文件，它的磁盘体积 100 447 B、
内联后是 98 698 B（1749 处 `\r` 被规范化）。产物 CRLF 计数恒为 **0**。

⇒ 因此「产物是否忠实」这个判据必须按**内容**比，不能按字节比 —— 这也正是 `--verify` 实现里
先剥掉 banner 时间戳、再比对完整文本的原因。
